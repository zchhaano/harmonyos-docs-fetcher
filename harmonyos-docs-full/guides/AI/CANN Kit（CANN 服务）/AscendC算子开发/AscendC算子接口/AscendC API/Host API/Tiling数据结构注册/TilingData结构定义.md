...
- set_+*field_name*接口：用于设置TilingData类的字段值，*field_name*为定义TilingData类时添加的字段名。
- SaveToBuffer接口：完成TilingData的序列化和保存。
- GetDataSize接口：获取TilingData的长度。

## 函数原型

- 定义一个TilingData类收起自动换行深色代码主题复制

```
BEGIN_TILING_DATA_DEF (class_name)
```
- 添加通用数据类型的TilingData字段收起自动换行深色代码主题复制

```
TILING_DATA_FIELD_DEF (data_type, field_name)
```
- 添加数组类型的TilingData字段，数组的元素数据类型为通用数据类型收起自动换行深色代码主题复制

```
TILING_DATA_FIELD_DEF_ARR (arr_type, arr_size, field_name)
```
- 添加结构体类型的TilingData字段收起自动换行深色代码主题复制
 ...
 **表1**BEGIN_TILING_DATA_DEF参数说明展开

| 参数 | 输入/输出 | 说明 |
| --- | --- | --- |
| class_name | 输入 | 开发者定义tiling结构体名，与c++变量命名要求一致。 |

  **表2**TILING_DATA_FIELD_DEF参数说明展开
...
| field_name | 输入 | 字段名，与c++变量命名要求一致。 |
  ...
  **表3**TILING_DATA_FIELD_DEF_ARR参数说明展开
...
| field_name | 输入 | 字段名，与c++变量命名要求一致。 |
  ...
  **表4**TILING_DATA_FIELD_DEF_STRUCT参数说明展开
...
- 使用SaveToBuffer接口时必须在set_+*field_name*接口后调用。
