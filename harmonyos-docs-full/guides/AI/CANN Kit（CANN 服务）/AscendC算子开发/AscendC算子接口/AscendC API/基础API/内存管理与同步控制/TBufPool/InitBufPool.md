 ...
 **表1**InitBufPool(T& bufPool, uint32_t len) 原型定义参数说明展开
...
| len | 输入 | 新划分资源池长度，单位为Byte，非32Bytes对齐会自动向上补齐至32Bytes对齐。 |
  ...
  **表2**InitBufPool(T& bufPool, uint32_t len, U& shareBuf) 原型定义参数说明展开
