...
Init主要用于对Matmul对象中的Tiling数据进行初始化，根据Tiling参数进行资源划分，tiling参数的具体介绍请参考[Matmul Tiling](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-matmul-tiling-class)。
...
```
// 开发者可以先通过REGIST_MATMUL_OBJ初始化单个matmul对象，不传入tiling参数，后续通过Init接口单独传入Tiling参数，对Tiling数据进行调整。 REGIST_MATMUL_OBJ (&pipe, GetSysWorkSpacePtr (), mm); mm. Init (&tiling);
```