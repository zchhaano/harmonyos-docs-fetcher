...
```
  ...
  - -c：ai_core-*<soc_version>*代表算子在AI Core上执行，*<soc_version>*为Kirin AI处理器的型号，可在运行环境通过命令进行查询:收起自动换行深色代码主题复制
...
```
  ...
  基于同系列的AI处理器型号创建的算子工程，其基础能力通用。命令执行完后，会在$HOME/sample目录下生成算子工程目录AddCustom，工程中包含算子实现的模板文件，编译脚本等，如下所示。收起自动换行深色代码主题复制
 ...
 **图1**核函数调用关系图
![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165255.72538456468873046097510223003419:50001231000000:2800:D4ED3FC787C1CC4A4703E2F93FED97461BFC2DDCE6D2EF425D07578C02309BA7.png) 
...
```
# ifndef ADD_CUSTOM_TILING_H # define ADD_CUSTOM_TILING_H # include "register/tilingdata_base.h" namespace optiling { BEGIN_TILING_DATA_DEF (AddCustomTilingData) // AddCustom算子使用了2个tiling参数：totalLength与tileNum TILING_DATA_FIELD_DEF ( uint32_t , totalLength); // 总计算数据量 TILING_DATA_FIELD_DEF ( uint32_t , tileNum); // 每个核上总计算数据分块个数 END_TILING_DATA_DEF; // 注册tiling数据到对应的算子 REGISTER_TILING_DATA_CLASS (AddCustom, AddCustomTilingData) } # endif // ADD_CUSTOM_TILING_H
```
...
在算子工程AddCustom目录下执行如下命令，进行算子工程编译。收起自动换行深色代码主题复制
