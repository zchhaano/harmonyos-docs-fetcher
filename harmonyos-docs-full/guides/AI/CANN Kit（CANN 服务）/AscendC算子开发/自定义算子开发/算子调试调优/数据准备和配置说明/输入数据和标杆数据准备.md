...
```
{ "gen_data" : false , "inputs" : [ { "data_file" : "/path/to/input_data_q.bin" } ] , "outputs" : [ { "data_file" : "/path/to/golden_data_out.bin" } ] }
```
...
```
{ "gen_data" : false , "inputs" : [ { "data_file" : "input_data_q.bin" } ] , "outputs" : [ { "data_file" : "golden_data_out.bin" } ] }
```
- **方式3：采用工具随机生成input数据。**
...
```
"data_script" : "${fa_case_dir}/flash_attention_score_golden.py" , "gen_data" : true , // ... "data_file" : "sample.bin" ,
```
