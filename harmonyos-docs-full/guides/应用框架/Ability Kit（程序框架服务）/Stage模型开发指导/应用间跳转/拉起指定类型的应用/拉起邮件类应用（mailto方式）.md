...
- 从应用拉起：       
  ...
  - 移动应用中，当用户点击“通过邮件分享”按钮时，应用会通过 mailto 调用邮件客户端，预填邮件主题和正文。         说明 
...
![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165939.19444401816433836190709992570547:50001231000000:2800:26CB75F929940769C8790B6FEC62B83ACF38935D239AA17C8364463B8C96643A.gif)
...
```
import { common } from '@kit.AbilityKit' ; @Entry @Component struct Index { build ( ) { Column () { Button ( '反馈' ) . onClick ( () => { let ctx = this . getUIContext (). getHostContext () as common. UIAbilityContext ; ctx. startAbility ({ action : 'ohos.want.action.sendToData' , uri : 'mailto:feedback@example.com?subject=App Feedback&body=Please describe your feedback here...' }) }) } } }
```

实现效果如下：

![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165939.71091828985438580589400956312863:50001231000000:2800:E0325BC53C1F4B9DFE7C7FE03F58D809C8CCB9E653E9CE4B855A98B69D0A0A48.gif)
...
```
{ "module" : { // ... "abilities" : [ { // ... "skills" : [ { "actions" : [ 'ohos.want.action.sendToData' ] , "uris" : [ { "scheme" : "mailto" , // linkFeature 用于适配垂类面板拉起 "linkFeature" : 'ComposeMail' } ] } ] } ] } }
```
...
```
export default class EntryAbility extends UIAbility { onCreate ( want : Want , launchParam : AbilityConstant . LaunchParam ): void { // 应用冷启动生命周期回调，其他业务处理... this . parseMailto (want); } onNewWant ( want : Want , launchParam : AbilityConstant . LaunchParam ): void { // 应用热启动生命周期回调，其他业务处理... this . parseMailto (want); } public parseMailto ( want: Want ) { const uri = want?. uri ; if (!uri || uri. length <= 0 ) { return ; } // 开始解析 mailto... } }
```