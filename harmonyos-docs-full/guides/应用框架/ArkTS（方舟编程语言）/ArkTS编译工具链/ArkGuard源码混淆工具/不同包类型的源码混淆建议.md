...
1. 开启混淆规则。建议开启[推荐的四项规则](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#推荐混淆功能)，其它选项按需添加。
...
4. 为保证与发布态三方库交互使用的正确性，ArkGuard会自动将工程级oh_modules目录下的远程HAR包的导出名称及其相关属性添加到不混淆名单中。由于开发者可以引用依赖模块中的任意路径下的文件，因此这种导出名称的收集范围并不局限于入口文件如Index.ets，而是工程级oh_modules中的所有文件中的导出名称。
...
1. HAR包的开发者需充分了解[三种混淆配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation-guide#三种混淆配置文件)以及[混淆规则的合并策略](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation#混淆规则合并策略)，以及在被HAP包使用时的[HAP包混淆建议](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#hap包混淆建议)中的注意事项，确保被应用依赖时本模块所有功能正常。
...
作为一个未发布的静态包，本地源码HAR包不会独立进行编译混淆，而是会跟随依赖它的主模块（如HAP）一同进行编译混淆，开发者需参阅[HAP包混淆建议](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#hap包混淆建议)了解相关行为。
...
1. 开启混淆规则。建议开启推荐的[四项混淆规则](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#推荐混淆功能)，其它选项按需添加。
...
1. HSP包的开发者需充分了解[三种混淆配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation-guide#三种混淆配置文件)以及[混淆规则的合并策略](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation#混淆规则合并策略)，以及在被HAP包使用时的[HAP包混淆建议](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#hap包混淆建议)中的注意事项，确保被应用依赖时本模块所有功能正常。
...
2. 对于模块内部混淆效果，开发者可以参阅[HAP包混淆建议](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#hap包混淆建议)中的所有建议。
...
2. 对于集成态HSP包的内部混淆效果，开发者可以参阅[HAP包混淆建议](/consumer/cn/doc/harmonyos-guides/source-obfuscation-practice#hap包混淆建议)中的所有建议。
