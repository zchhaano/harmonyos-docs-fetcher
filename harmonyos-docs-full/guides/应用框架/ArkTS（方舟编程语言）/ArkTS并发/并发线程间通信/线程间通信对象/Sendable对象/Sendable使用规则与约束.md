...
```
class A { constructor ( ) { } } @Sendable class B extends A { // A不是sendable class，B不能继承它，编译报错 constructor ( ) { super () } }
```
...
```
@Sendable class A { constructor ( ) { } } class B extends A { // A是sendable class，B不能继承它，编译报错 constructor ( ) { super () } }
```
...
```
@Sendable class A { constructor ( ) { } b : Array < number > = [ 1 , 2 , 3 ] // 编译报错，需使用collections.Array }
```
...
```
@Sendable class A { constructor ( ) { } a!: number ; // 编译报错，不支持使用“!” }
```
...
```
enum B { b1 = "bbb" } @Sendable class A { [ "aaa" ]: number = 1 ; // 编译报错，不支持["aaa"] [B. b1 ]: number = 2 ; // 编译报错，不支持[B.b1] }
```
...
```
@Sendable class B { num1 : number = 1 ; num2 : number = 2 ; add (): number { return this . num1 + this . num2 ; } }
```
...
```
type A = number ; @Sendable class B { num1 : A = 1 ; // 运行报错，不支持使用类型别名 num2 : A = 2 ; // 运行报错，不支持使用类型别名 add (): number { return this . num1 + this . num2 ; } }
```
...
```
import { collections } from '@kit.ArkTS' ; try { let arr1 : collections. Array < Array < number >> = new collections. Array < Array < number >>(); // 编译报错，模板类型必须是Sendable类型 let arr2 : Array < number > = new Array < number >(); arr2. push ( 1 ); arr1. push (arr2); } catch (e) { console . error ( `taskpool execute: Code: ${e.code} , message: ${e.message} ` ); }
```
...
```
import { lang } from '@kit.ArkTS' ; type ISendable = lang. ISendable ; interface I extends ISendable {} @Sendable class B implements I {} function bar ( ): B { return new B (); } let b = new B (); { @Sendable class A implements I {} @Sendable class C { u : I = bar (); // bar不是sendable class对象，编译报错 v : I = new A (); // A不是定义在top level中，编译报错 foo ( ) { return b; // b不是sendable class对象，而是sendable class的实例，编译报错 } } }
```
...
```
@Sendable type A = number ; // 编译报错 @Sendable type D = C; // 编译报错
```
...
```
@Sendable @Observed // 编译报错 class C { num : number = 1 ; }
```
...
```
"buildOption" : { "strictMode" : { "caseSensitiveCheck" : true , "useNormalizedOHMUrl" : true , "disableSendableCheckRules" : [ "arkts-sendable-class-decorator" ] } }
```
...
```
class A { state : number = 0 ; } @Sendable class SendableA { state : number = 0 ; } let a2 : SendableA = new A () as SendableA ; // 编译报错
```
...
```
{ "module" : { "name" : "library" , "type" : "har" , "deviceTypes" : [ "tablet" , "2in1" ], "metadata" : [ { "name" : "UseTsHar" , "value" : "true" } ] } }
```
