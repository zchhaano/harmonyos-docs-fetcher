...
![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165429.10725067390062126441038231724505:50001231000000:2800:31B6E579B447EFDCA4376D8AD319538BF7F311D3DCD5F0BE0220035B7116E1E5.png)
...
```
// 通过Mimetype创建解码器，这里示例创建的是AAC编码格式，第二个入参设置 false 表示当前是解码。 OH_AVCodec *audioDec_ = OH_AudioCodec_CreateByMime(OH_AVCODEC_MIMETYPE_AUDIO_AAC, false );
```
...
```
// 通过codec name创建解码器。 OH_AVCapability *capability = OH_AVCodec_GetCapability (OH_AVCODEC_MIMETYPE_AUDIO_AAC, false ); const char *name = OH_AVCapability_GetName (capability); OH_AVCodec *audioDec_ = OH_AudioCodec_CreateByName (name);
```
...
![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165429.69801737883855917201397806204781:50001231000000:2800:1EA0C8708BBBF581ADD599D06FA211BC8AB3C01902E763258655D7756554B72A.png)

各音频解码类型参数范围说明：

![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165429.64904741479422769057864910614472:50001231000000:2800:FC32A438007A4890D5C9DD02B5D38ABC0747D1FD0458654E4C14DC2A2D325A33.png)
...
```
OH_AVErrCode ret = OH_AudioCodec_Prepare(audioDec_); if (ret != AV_ERR_OK) { // 异常处理。 }
```
...
```
// 刷新解码器audioDec_。 OH_AVErrCode ret = OH_AudioCodec_Flush(audioDec_); if (ret != AV_ERR_OK ) { // 异常处理。 } // 重新开始解码。 ret = OH_AudioCodec_Start(audioDec_); if (ret != AV_ERR_OK ) { // 异常处理。 }
```
...
```
// 重置解码器audioDec_。 OH_AVErrCode ret = OH_AudioCodec_Reset(audioDec_); if (ret != AV_ERR_OK ) { // 异常处理。 } // 重新配置解码器参数。 ret = OH_AudioCodec_Configure(audioDec_, format); if (ret != AV_ERR_OK ) { // 异常处理。 }
```
...
```
// 终止解码器audioDec_。 OH_AVErrCode ret = OH_AudioCodec_Stop(audioDec_); if (ret != AV_ERR_OK) { // 异常处理。 }
```
...
```
// 调用OH_AudioCodec_Destroy, 销毁解码器。 OH_AVErrCode ret = OH_AudioCodec_Destroy(audioDec_); if (ret != AV_ERR_OK ) { // 异常处理。 } else { audioDec_ = NULL ; // 不可重复destroy。 }
```