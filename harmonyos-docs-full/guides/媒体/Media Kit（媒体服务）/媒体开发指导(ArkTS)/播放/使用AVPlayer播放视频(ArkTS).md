...
![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165448.84586670738467480536528672749210:50001231000000:2800:DB8533DE4CF8214437070C0C8FB64C732AFDBE7B786637446D4685F82694ED1D.png)
...
```
let url = 'https://xxx.xxx.xxx.mp4' ; if (avPlayer == null ) { return ; } avPlayer. url = url;
```
...
```
// 通过接口getXComponentSurfaceId获取surfaceId。 let surfaceId = '' ; if (avPlayer == null ) { return ; } if (surfaceId === '' ) { return ; } avPlayer. surfaceId = surfaceId;
```
...
```
import { BusinessError } from '@kit.BasicServicesKit' ; avPlayer. reset ( ( err: BusinessError ) => { avPlayer. url = url; if (err) { console . error ( 'Failed to reset,error message is :' + err. message ); } else { console . info ( 'Succeeded in resetting' ); } }); // 更换url。 let url = 'https://xxx.xxx.xxx.mp4' ; if (avPlayer == null ) { return ; } avPlayer. url = url;
```
...
1. 新建工程，下载[示例工程](https://gitcode.com/openharmony/applications_app_samples/tree/master/code/DocsSample/Media/AVPlayer/AVPlayerArkTSVideo)，并将示例工程的以下资源复制到对应目录。       收起自动换行深色代码主题复制

```
AVPlayerArkTSVideo entry/src/main/ets/ └── pages └── Index.ets (播放界面) entry/src/main/resources/ ├── base │   ├── element │   │   ├── color.json │   │   ├── float.json │   │   └── string.json │   └── media │       ├── ic_video_play.svg  (播放键图片资源) │       └── ic_video_pause.svg (暂停键图片资源) └── rawfile └── test1.mp4 （视频资源）
```
2. 编译新建工程并运行。