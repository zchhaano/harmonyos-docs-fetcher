...
```
SLDataLocator_BufferQueue slBufferQueue = { SL_DATALOCATOR_BUFFERQUEUE, 1 }; // 具体参数需要根据音频文件格式进行适配。 SLDataFormat_PCM pcmFormat = { SL_DATAFORMAT_PCM, 2 , // 通道数。 SL_SAMPLINGRATE_48, // 采样率。 SL_PCMSAMPLEFORMAT_FIXED_16, // 音频采样格式。 16 , SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT, SL_BYTEORDER_LITTLEENDIAN }; SLDataSource slSource = { &slBufferQueue, &pcmFormat }; SLObjectItf pcmPlayerObject = nullptr ; (*engineEngine)-> CreateAudioPlayer (engineEngine, &pcmPlayerObject, &slSource, &slSink, 0 , nullptr , nullptr ); (*pcmPlayerObject)-> Realize (pcmPlayerObject, SL_BOOLEAN_FALSE);
```
