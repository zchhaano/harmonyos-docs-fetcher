...
```
// 创建一块公共缓存buffer，用于写入录制数据和读取播放数据。 // 自定义读入录制数据函数。 int32_t MyOnReadData ( OH_AudioCapturer* capturer, void * userData, void * buffer, int32_t length) { // 从buffer中取出length长度的录音数据，放在创建好的公共缓存buffer中，用于供renderer进行读取。 return 0 ; } // 自定义写入数据函数。 int32_t MyOnWriteData ( OH_AudioRenderer* renderer, void * userData, void * buffer, int32_t length) { // 从公共缓存buffer中读取数据，并按length长度写入buffer。 return 0 ; }
```
