...
| --- | --- | --- |
| HAR | HAR中的代码和资源跟随使用方编译，如果有多个使用方，它们的编译产物中会存在多份相同拷贝。 注意： 构建HAR 时，建议开启混淆能力，保护代码资产。 | HAR除了支持应用内引用，还可以独立打包发布到 OHPM中心仓 或者 OHPM私仓 ，供其他应用引用。 |
| HSP | HSP中的代码和资源可以独立编译，运行时在一个进程中代码也只会存在一份。 | HSP一般随应用进行打包，当前支持应用内和 集成态HSP 。应用内HSP只支持应用内引用，集成态HSP支持发布到OHPM私仓和跨应用引用。 说明： 集成态HSP只是应用内HSP的中间形态，只能参与编译构建过程，无法单独安装。在构建和发布OHPM私仓的过程中，集成态HSP不与特定的应用包名耦合。使用时，工具链支持自动将集成态HSP的包名替换成宿主应用包名，并且会重新签名生成一个新的HSP包，作为宿主应用的安装包，这个新的HSP也属于宿主应用HAP的应用内HSP。 |

**图1** HAR和HSP在APP包中的形态示意图

![image](https://alliance-communityfile-drcn.dbankcdn.com/FileServer/getFile/cmtyPub/011/111/111/0000000000011111111.20260224165342.45252110122064712456322270237934:50001231000000:2800:EB850373ACA728CDAFC88DB72D396549FA50D2C573F2A9406A929524D5855E44.png)
...
| Static Library | HAR | 静态共享包，编译态复用。 - 支持应用内共享，也可以作为二方库（SDK）、三方库（SDK）发布后供其他应用使用。 - 作为二方库（SDK），发布到 OHPM私仓 ，供公司内部其他应用使用。 - 作为三方库（SDK），发布到 OHPM中心仓 ，供其他应用使用。 - 多包（HAP/HSP）同时引用相同的HAR时，会造成多包间代码和资源的重复拷贝，从而导致应用包增大。 - 注意： 构建HAR 时，建议开启混淆能力，保护代码资产。 |
...
| 规格 | HAP | HAR | HSP |
| --- | --- | --- | --- |
| 支持在配置文件中声明 UIAbility 组件 | √ | √ | √ |
| 支持在配置文件中声明 ExtensionAbility 组件 | √ | √ | √ |
| 支持在配置文件中声明 pages 页面 | √ | × | √ |
| 支持包含资源文件与.so文件 | √ | √ | √ |
| 支持依赖其他HAR文件 | √ | √ | √ |
| 支持依赖其他HSP文件 | √ | √ | √ |
...
- 如果HAR支持声明pages页面，那么当HAR被打包到HAP或HSP中时，其内部声明的pages页面可能会与HAP/HSP中的pages页面存在相对路径上的重复，这将导致无法根据相对路径识别特定的路由页面。因此，HAR不支持在配置文件中声明pages页面，但可以包含pages页面，并通过[Navigation跳转](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-navigation#路由操作)的方式进行跳转。
