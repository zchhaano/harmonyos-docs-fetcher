...
```
// 类型注解（类似Swift）。 let age : number = 20 ; const program : string = 'ArkTS' ; // 类型推断（类似Swift的局部变量类型推断）。 let version = 5.0 ;
```
...
```
// 常规函数定义，与Swift类似。 function add ( x: number , y: number ): number { return x + y; } // 简洁的箭头函数形式，类似Swift的闭包语法。 const multiply = ( a : number , b : number ): number => a * b;
```
...
```
function foo ( name?: string ){} /*  name为可选参数。  */ foo ( 'hello' ); //  OK，传入name参数。 foo (); //  OK，不传name参数。
```
...
```
class MyClass { func method () { print ( self ) // 始终指向MyClass的实例。 } }
```
...
```
class A { bar : string = 'I am A' ; foo ( ) { console . info ( this . bar ); } } class B { bar : string = 'I am B' ; callFunction ( fn: () => void ) { fn (); } } function callFunction ( fn: () => void ) { fn (); } let a : A = new A (); let b : B = new B (); callFunction (a. foo ); // 程序crash。this的上下文发生了变化。 b. callFunction (a. foo ); // 程序crash。this的上下文发生了变化。 b. callFunction (a. foo . bind (b)) // 输出'I'm B'。
```
...
```
interface Person { name : string ; age?: number ; // age是可选属性。 } const person : Person = { name : "Alice" , };
```
