...
```
// code-linter.json5 { "rules" : { "@cross-device-app-dev/grid-columns-span" : "warn" } }
```
...
```
@Entry @Component struct Index { build () { Column () { GridRow ({ columns : { sm : 4 , md : 8 , lg : 12 } }) { GridCol ({ span: { sm: 4, md: 8, lg: 12 } }) { Row () .backgroundColor ($ r ( 'sys.color.ohos_id_color_palette_aux1' )) } } } } }
```
