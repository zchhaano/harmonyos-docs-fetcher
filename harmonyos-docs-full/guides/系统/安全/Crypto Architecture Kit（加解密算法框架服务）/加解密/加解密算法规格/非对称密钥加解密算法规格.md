...
- [NoPadding](/consumer/cn/doc/harmonyos-guides/crypto-asym-encrypt-decrypt-spec#填充模式为nopadding)：不带填充，输入的数据必须与RSA密钥字节长度一样长；输出数据长度与RSA密钥字节长度一样长。
- [PKCS1](/consumer/cn/doc/harmonyos-guides/crypto-asym-encrypt-decrypt-spec#填充模式为pkcs1)：即RFC3447规范中的RSAES-PKCS1-V1_5模式，对应OpenSSL中的RSA_PKCS1_PADDING。

RSA运算时，源数据D转化为Encryption block（EB），加密时，输入数据最大长度 <= RSA密钥字节长度 - 11；输出数据长度与RSA密钥字节长度相同。
- [PKCS1_OAEP](/consumer/cn/doc/harmonyos-guides/crypto-asym-encrypt-decrypt-spec#填充模式为pkcs1_oaep)：即RFC3447规范中的RSAES-OAEP模式，对应OpenSSL中的RSA_PKCS1_OAEP_PADDING。
