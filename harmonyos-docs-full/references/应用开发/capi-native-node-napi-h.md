...
将ArkTS侧创建的[$r](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-resource)资源对象映射到Native侧的[ArkUI_DrawableDescriptor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-drawabledescriptor)。
...
| int32_t* writeLength | 在返回 ARKUI_ERROR_CODE_NO_ERROR 时表示实际写入到缓冲区的字符串长度。 在返回 ARKUI_ERROR_CODE_BUFFER_SIZE_ERROR 时表示可以容纳目标的最小缓冲区大小。 |
...
| 参数项 | 描述 |
| --- | --- |
| ArkUI_ContextHandle uiContext | UIContext 对象指针，用以绑定实例。 |
