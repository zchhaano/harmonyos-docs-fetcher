...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceId | string | 是 | 连接的目标设备的deviceId，即对端设备的BLE MAC地址。BLE MAC的获取方法，请参考 查找设备 。 |
...
| callback | Callback< Connection > | 是 | 注册的回调函数。 Connection 返回的连接对象。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'connectionAccepted'，收到对端连接，触发该事件。 |
| callback | Callback< Connection > | 否 | 注册的回调函数。 Connection 返回的连接对象。 |
...
| reason | number | 否 | 否 | 连接成功返回0，连接失败返回错误码： - 32390200：表示客户端连接超时。 - 32390201：表示服务端服务未启动。 - 32390300：表示内部错误。 更多关于错误码的详细介绍请参考 增强连接错误码 。 |
