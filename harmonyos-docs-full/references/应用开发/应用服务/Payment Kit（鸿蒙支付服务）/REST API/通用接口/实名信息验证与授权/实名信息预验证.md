...
| --- | --- |
| 接口方向 | 开发者服务器 -> 华为支付服务器 |
| 接口URL | https://petalpay-developer.cloud.huawei.com.cn/api/v1/realname/verification/preverify 说明 元服务接口调用一致。 |
...
| openId | 是 | String | 用户对外展示的ID（获取方式请参见 获取用户信息 ）。长度范围为1-512。 注意 入参中的实名信息，必须和 openId用户下的实名信息保持一致。 |
