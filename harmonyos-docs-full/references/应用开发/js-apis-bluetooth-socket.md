...
需要在服务端调用完[socket.sppListen](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#socketspplisten)后调用该接口，且传入的链路类型[SppType](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)需是SPP_L2CAP或SPP_L2CAP_BLE。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| serverSocket | number | 是 | 服务端套接字的ID。 该值是调用 socket.sppListen 接口后，通过其异步callback获取到的。 |
...
| 类型 | 说明 |
| --- | --- |
| number | 返回L2CAP链路类型套接字的psm值。 - SppType 设置为SPP_L2CAP_BLE时，返回值的有效值范围为[0x01, 0xFF]。 - SppType 设置为SPP_L2CAP时，返回值的有效值范围为[0x0000, 0xFFFF]。 - 服务端通道建立异常或 SppType 非L2CAP链路类型时，返回-1。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| clientSocket | number | 是 | 客户端套接字的ID。 该值是调用 socket.sppAccept 或 socket.sppConnect 接口后，通过其异步callback获取到的。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| socket | number | 是 | 服务端监听套接字的ID。 该值是调用 socket.sppListen 接口，通过其异步callback获取到的。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| socket | number | 是 | 客户端套接字的ID。 该值是调用 socket.sppAccept 或 socket.sppConnect 接口，通过其异步callback获取到的。 |
...
- 链路类型为[SPP_L2CAP](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)或[SPP_L2CAP_BLE](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，单次发送数据大小存在限制，单位为Byte，超过限制范围的数据将被截断。链路类型为SPP_L2CAP_BLE时单次发送数据大小范围为[1, 65535]；链路类型为SPP_L2CAP时服务端单次发送数据大小范围为[1, 4091]，客户端单次发送数据大小范围为[1, 8085]。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| clientSocket | number | 是 | 客户端套接字的ID。 该值是调用 socket.sppAccept 或 socket.sppConnect 接口，通过其异步callback获取到的。 |
...
- 链路类型为[SPP_RFCOMM](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，当接收数据大小超过1024字节时，会自动分多次接收，每次接收数据大小范围为[1, 1024]。
- 链路类型为[SPP_L2CAP](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)或[SPP_L2CAP_BLE](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，单次接收数据大小存在限制，需一次性读取全部数据，单位为Byte，超过最大接收数据大小限制将被截断。链路类型为SPP_L2CAP_BLE时单次接收数据大小范围为[1, 65535]；链路类型为SPP_L2CAP时服务端和客户端单次接收数据大小范围为[1, 8087]。
...
| clientSocket | number | 是 | 客户端套接字的ID。 该值是调用 socket.sppAccept 或 socket.sppConnect 接口，通过其异步callback获取到的。 |
| callback | Callback<ArrayBuffer> | 否 | 指定取消订阅的回调函数通知。 若传参，则需与 socket.on('sppRead') 中的回调函数一致；若无传参，则取消订阅该type对应的所有回调函数通知。 |
...
- 按照蓝牙协议规范，数据通道在空闲状态需进入休眠模式以降低功耗。蓝牙子系统实现上，通道在5-7s内没有数据交互时会进入休眠模式，将导致下次调用此接口发送数据前，会耗费500ms左右退出休眠模式才开始发送数据。
- 若想减少每次发送数据前退休眠模式的耗时，建议每3s左右可往数据通道上发送一次任意大小的心跳数据，对数据通道进行保活，可防止进入休眠模式，但同时也会提高设备功耗。
- 链路类型为[SPP_L2CAP](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)或[SPP_L2CAP_BLE](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，单次发送数据大小存在限制，单位为Byte，超过限制大小的数据将被截断。链路类型为SPP_L2CAP_BLE时单次发送数据大小范围为[1, 65535]；链路类型为SPP_L2CAP时服务端单次发送数据大小范围为[1, 4091]，客户端单次发送数据大小范围为[1, 8085]。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| clientSocket | number | 是 | 客户端套接字的ID。 该值是调用 sppAccept 或 sppConnect 接口，通过其异步callback获取到的。 |
| data | ArrayBuffer | 是 | 写入的数据。 |
...
- 若客户端使用，需在调用[socket.sppConnect](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#socketsppconnect)后，且连接成功后使用。
- 若服务端使用，需在调用[socket.sppAccept](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#socketsppaccept)后，且连接成功后使用。
- 若套接字链路类型为[SPP_RFCOMM](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，最大接收数据大小无限制且返回值为0。
...
- 若客户端使用，需在调用[socket.sppConnect](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#socketsppconnect)后，且连接成功后使用。
- 若服务端使用，需在调用[socket.sppAccept](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#socketsppaccept)后，且连接成功后使用。
- 若套接字链路类型为[SPP_RFCOMM](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket#spptype)时，最大发送数据大小无限制且返回值为0。
