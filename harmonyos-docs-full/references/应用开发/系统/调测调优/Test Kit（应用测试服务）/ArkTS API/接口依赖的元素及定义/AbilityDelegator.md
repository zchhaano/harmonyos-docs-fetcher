...
| 类型 | 说明 |
| --- | --- |
| Promise< ShellCmdResult > | Promise对象，返回Shell命令执行结果 ShellCmdResult 对象。 |
...
### removeAbilityStageMonitorSync 10+
...
| --- | --- | --- | --- |
| monitor | AbilityStageMonitor | 是 | AbilityStageMonitor 实例。 |
| callback | AsyncCallback<AbilityStage> | 是 | 回调函数。当等待并返回与给定AbilityStageMonitor中设置的条件匹配的AbilityStage对象的操作成功，err为undefined，data为获取到的 AbilityStage 对象；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<AbilityStage> | Promise对象，返回 AbilityStage 对象。 |
