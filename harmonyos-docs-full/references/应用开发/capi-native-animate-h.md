...
| --- | --- |
| ArkUI_AnimateOption * option | 动画效果参数。 |
| float value | 动画播放速度。 说明： 传入小于0的数值，会默认设置为1。 |
...
此方法优先于[OH_ArkUI_AnimateOption_SetCurve](/consumer/cn/doc/harmonyos-references/capi-native-animate-h#oh_arkui_animateoption_setcurve)生效。
...
| 类型 | 说明 |
| --- | --- |
| float | 动画插值结果。 说明： 在动画过程中，插值结果根据动画参数在插值起点 float OH_ArkUI_AnimatorOption_GetBegin(ArkUI_AnimatorOption* option) 和插值终点 float OH_ArkUI_AnimatorOption_GetEnd(ArkUI_AnimatorOption* option) 间变化。 |
