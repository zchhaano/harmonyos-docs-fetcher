...
| abilityInfos | Array< AbilityInfo > | 是 | 否 | Ability的配置信息 通过调用 bundle.getBundleInfo 接口时，传入GET_BUNDLE_WITH_ABILITIES获取。 |
| reqPermissions | Array<string> | 是 | 否 | 应用运行时需向系统申请的权限集合 通过调用 bundle.getBundleInfo 接口时，传入GET_BUNDLE_WITH_REQUESTED_PERMISSION获取。 |
| reqPermissionDetails | Array< ReqPermissionDetail > | 是 | 否 | 应用运行时需向系统申请的权限集合的详细信息 通过调用 bundle.getBundleInfo 接口时，传入GET_BUNDLE_WITH_REQUESTED_PERMISSION获取。 |
...
| reqPermissionStates 8+ | Array<number> | 是 | 否 | 申请权限的授予状态。0表示申请成功，-1表示申请失败。 |

## ReqPermissionDetail (deprecated)

支持设备PhonePC/2in1TabletTVWearable说明 

从API version 7开始支持，从API version 9开始废弃，建议使用[ReqPermissionDetail](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-bundleinfo)替代。
