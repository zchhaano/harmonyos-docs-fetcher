...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<void> | 是 | 回调函数。当以默认设置触发拍照成功，err为undefined，否则为错误对象。错误码类型 CameraErrorCode 。 |
...
| callback | AsyncCallback<void> | 是 | 回调函数，用于获取结果。接口调用失败会返回相应错误码，错误码类型 CameraErrorCode 。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| codecType | VideoCodecType | 是 | 动态照片短视频编码类型。 |
...
| 类型 | 说明 |
| --- | --- |
| ImageRotation | 获取拍照旋转角度。若接口调用失败，返回undefined。 |
