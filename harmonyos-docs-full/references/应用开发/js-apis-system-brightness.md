...
从API version 7开始不再维护，建议使用[window.setWindowKeepScreenOn()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowkeepscreenon9)替代。
...
| 名称 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| success | (data: BrightnessResponse ) => void | 否 | 接口调用成功的回调函数。data为 BrightnessResponse 类型的返回值。 |
...
| 名称 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| success | (data: BrightnessModeResponse ) => void | 否 | 接口调用成功的回调函数。data为 BrightnessModeResponse 类型的返回值。 |
| fail | (data: string, code: number) => void | 否 | 接口调用失败的回调函数。data为错误信息，code为错误码。 |
| complete | () => void | 否 | 接口调用结束的回调函数。 |
