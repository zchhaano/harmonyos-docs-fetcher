...
| 类型 | 说明 |
| --- | --- |
| number | 返回连接id，客户端可以通过 disconnectServiceExtensionAbility 传入该连接id来断开连接。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<UIServiceProxy> | Promise对象，连接UIServiceExtensionAbility成功时，返回 UIServiceProxy 对象，借助该对象可以往UIServiceExtensionAbility发送数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| proxy | UIServiceProxy | 是 | connectUIServiceExtensionAbility 返回的Proxy。 |
