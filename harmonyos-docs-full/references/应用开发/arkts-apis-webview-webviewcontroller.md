...
风险提示：如果想获取url来做JavascriptProxy通信接口认证，请使用[getLastJavascriptProxyCallingFrameUrl 12+](/consumer/cn/doc/harmonyos-references/arkts-apis-webview-webviewcontroller#getlastjavascriptproxycallingframeurl12)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| userAgent | string | 是 | 用户自定义代理信息。建议先使用 getUserAgent 获取当前默认用户代理，在此基础上追加自定义用户代理信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| userAgent | string | 是 | 用户自定义代理信息。建议先使用 getDefaultUserAgent 获取当前默认用户代理，在此基础上追加自定义用户代理信息。 |
...
## enableIntelligentTrackingPrevention 12+
...
## isIntelligentTrackingPreventionEnabled 12+
...
## addIntelligentTrackingPreventionBypassingList 12+
...
## removeIntelligentTrackingPreventionBypassingList 12+
...
## clearIntelligentTrackingPreventionBypassingList 12+
...
## isAdsBlockEnabledForCurPage 12+
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | RenderProcessMode | 是 | 渲染子进程模式。 可以先调用 getRenderProcessMode() 查看当前设备的ArkWeb渲染子进程模式，枚举值0为单子进程模式，枚举值1为多子进程模式。 手机默认为单渲染子进程模式，平板和PC/2in1默认为多渲染子进程模式。 如果传入RenderProcessMode枚举值之外的非法数字，则默认识别为多渲染子进程模式。 |
...
## createWebPrintDocumentAdapter 11+
...
## getLastJavascriptProxyCallingFrameUrl 12+
...
## setServiceWorkerWebSchemeHandler 12+
...
## clearServiceWorkerWebSchemeHandler 12+
...
## setPathAllowingUniversalAccess 12+
...
| 类型 | 说明 |
| --- | --- |
| Promise< ControllerAttachState > | Promise实例，返回当前 ControllerAttachState 状态。 |
...
| port | number | 是 | 指定DevTools服务的tcp端口号。如果没有指定port，那么该接口等同于 setWebDebuggingAccess 接口。 取值范围: (1024, 65535] 如果port的值在区间[0, 1024]内，则会抛出BusinessError异常，错误码为17100023。 |
...
## isPrivateNetworkAccessEnabled 20+
...
| 类型 | 说明 |
| --- | --- |
| WebBlanklessErrorCode | 返回接口调用是否成功，具体见 WebBlanklessErrorCode 定义。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keys | Array<string> | 否 | 清除Blankless优化方案页面的key值列表，key值为 getBlanklessInfoWithKey 中指定过的。 默认值：所有Blankless优化方案缓存的页面key列表。 合法取值范围：长度不超过2048，key列表长度<=100。key和加载页面时输入给ArkWeb的相同。 非法值设置行为：传入undefined/null会抛出异常错误码401；key长度超过2048时该key不生效；长度超过100时，取前100个；当为空时，使用默认值。 |
...
## setBlanklessLoadingCacheCapacity 20+
...
| 类型 | 说明 |
| --- | --- |
| ArkWebEngineVersion | 返回由 ArkWebEngineVersion 所定义的当前使用的ArkWeb内核版本。 |
...
## setSoftKeyboardBehaviorMode 22+
