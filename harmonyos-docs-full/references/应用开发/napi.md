...
- [in] js_object：ArkTS对象。
- [in] native_object：将被包裹在ArkTS对象中的native实例。
- [in] finalize_cb：[可选]ArkTS对象被销毁时调用的回调函数，详情请参见[napi_finalize回调函数说明](/consumer/cn/doc/harmonyos-references/napi#napi_finalize回调函数说明)。
...
- [in] str：指向外部字符串的指针。
- [in] length：字符串长度。
- [in] finalize_callback：[可选]字符串对象被销毁时调用的回调函数，详情请参见[napi_finalize_callback回调函数说明](/consumer/cn/doc/harmonyos-references/napi#napi_finalize_callback回调函数说明)。
- [in] finalize_hint：[可选]上下文提示，会传递给回调函数。
