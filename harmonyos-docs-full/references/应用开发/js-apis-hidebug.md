...
## hidebug.getAppVMObjectUsedSize 21+
...
## hidebug.getAppThreadCpuUsage 12+
...
## hidebug.startAppTraceCapture 12+
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tags | number[] | 是 | trace范围，详情请见 tags 。 |
| flag | TraceFlag | 是 | 详情请见 TraceFlag 。 |
...
| 类型 | 说明 |
| --- | --- |
| string | 返回trace文件名路径（接口返回真实物理路径，若应用内需要访问，请参考 应用沙箱路径和真实物理路径的对应关系 进行路径转换）。 |
...
## hidebug.stopAppTraceCapture 12+
...
## hidebug.setAppResourceLimit 12+
...
## hidebug.getAppNativeMemInfo 12+
...
由于读取/proc/{pid}/smaps_rollup耗时较长，推荐使用异步接口[hidebug.getAppNativeMemInfoAsync](/consumer/cn/doc/harmonyos-references/js-apis-hidebug#hidebuggetappnativememinfoasync20)，以避免应用丢帧或卡顿。
...
## hidebug.getAppNativeMemInfoAsync 20+
...
## hidebug.getAppNativeMemInfoWithCache 20+
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| rssLimit | bigint | 否 | 否 | 应用程序进程可用的物理内存限制，以KB为单位，实际当前系统未对进程可用物理内存做限制，但是进程的可用物理内存仍然不会超过设备的实际最大可用物理内存，当前设备的物理内存使用情况可通过 hidebug.getSystemMemInfo 获取。 |
...
## hidebug.getGraphicsMemorySync 14+
...
## hidebug.getGraphicsMemorySummary 21+
...
## hidebug.enableGwpAsanGrayscale 20+
...
## hidebug.disableGwpAsanGrayscale 20+
...
## hidebug.getGwpAsanGrayscaleState 20+
...
## hidebug.setJsRawHeapTrimLevel 20+
