...
| PreferStrategy | PreferStrategy | 偏好策略。在 OH_LOG_SetLogLevel 中使用。不同策略，实际生效的最低日志级别也不同。 |
...
偏好策略。在[OH_LOG_SetLogLevel](/consumer/cn/doc/harmonyos-references/capi-log-h#oh_log_setloglevel)中使用。不同策略，实际生效的最低日志级别也不同。
...
| 参数项 | 描述 |
| --- | --- |
| const LogType type | 日志类型，三方应用日志类型为 LOG_APP 。 |
