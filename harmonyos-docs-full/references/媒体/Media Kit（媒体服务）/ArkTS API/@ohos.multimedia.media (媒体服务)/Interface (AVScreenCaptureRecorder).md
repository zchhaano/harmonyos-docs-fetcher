...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| windowIDs | Array<number> | 是 | 需要豁免隐私的窗口列表，包括主窗口id和子窗口id，窗口属性获取方法可以参考 getWindowProperties 。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| excludedWindows | Array<number> | 是 | 需要在Picker中隐藏的窗口列表，窗口属性获取方法可以参考 getWindowProperties 。 |
...
| callback | Callback< AVScreenCaptureStateCode > | 是 | 状态切换事件回调方法， AVScreenCaptureStateCode 表示切换到的状态。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 状态切换事件回调类型，支持的事件：'stateChange'。 |
| callback | Callback< AVScreenCaptureStateCode > | 否 | 状态切换事件回调方法， AVScreenCaptureStateCode 表示切换到的状态，不填此参数则会取消最后一次订阅事件。 |
