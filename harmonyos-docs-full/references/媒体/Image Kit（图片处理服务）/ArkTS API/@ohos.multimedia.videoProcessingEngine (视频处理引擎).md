...
本模块包含一个基础类：[ImageProcessor](#imageprocessor)类。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8a/v3/EnNaiMdTSkiKCtNd9ORCew/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194155Z&HW-CC-Expire=86400&HW-CC-Sign=343AD6A39B0CBC77E32D7E7232710D30C08C34FE677A0DD6EEBCA80E0CD3BEAC)   
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sourceImage | image.PixelMap | 是 | 输入图像，内存类型需为DMA内存，具体情况请参考 PixelMap的内存类型介绍 。 |
...
| --- | --- | --- | --- |
| sourceImage | image.PixelMap | 是 | 输入图像，内存类型需为DMA内存，具体情况请参考 PixelMap的内存类型介绍 。 |
| scale | number | 是 | 目标缩放比例。取值范围(0.0, 32.0]（在满足 上述分辨率范围 的前提下，最高支持32倍放大）。 |
| level | QualityLevel | 否 | 算法档位（HIGH、MEDIUM、LOW、NONE），默认为NONE。 |
