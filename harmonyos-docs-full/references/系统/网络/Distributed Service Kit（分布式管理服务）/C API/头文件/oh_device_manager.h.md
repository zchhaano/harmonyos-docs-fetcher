...
| 类型 | 说明 |
| --- | --- |
| int32_t | 返回执行的错误码。错误码定义详见 DeviceManager_ErrorCode 。 返回 ERR_OK ，表示执行成功。 返回 DM_ERR_FAILED ，表示函数执行失败。 返回 DM_ERR_OBTAIN_SERVICE ，表示获取设备管理服务失败。 返回 DM_ERR_OBTAIN_BUNDLE_NAME ，表示获取bundleName失败。 返回 ERR_INVALID_PARAMETER ，表示参数localDeviceName是空指针或者*localDeviceName是非空指针。 |
