...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tagInfo | TagInfo | 是 | 包含Tag技术类型和相关参数，从 tag.getTagInfo(want: Want) 获取。 |
...
## tag.registerForegroundDispatch 10+
...
| discTech | number[] | 是 | 前台应用指定的NFC读卡技术类型，不可以为空，至少指定一种读卡技术类型。每个number值表示所支持技术类型的常量值型，根据number值设置NFC读卡轮询的Tag技术类型（仅包含 NFC_A , NFC_B , NFC_F , NFC_V 中的一种或多种）。 |
...
## tag.unregisterForegroundDispatch 10+
...
## tag.ndef.makeApplicationRecord 18+
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| uid 9+ | number[] | 否 | 否 | 标签的uid，每个number值是十六进制表示，范围是0x00~0xFF。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| technology 9+ | number[] | 否 | 否 | 支持的技术类型，每个number值表示所支持技术类型的常量值。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| supportedProfiles (deprecated) | number[] | 否 | 否 | 支持的技术类型。 说明： 从API version 7开始支持，从API version 9开始废弃，使用 tag.TagInfo#technology 替代。 |
...
| NDEF_FORMATABLE 9+ | number | 7 | 可以格式化的NDEF技术。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| NFC_BARCODE 18+ | number | 10 | BARCODE技术。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 值 | 说明 |
| --- | --- | --- | --- |
| RTD_TEXT 9+ | number[] | [0x54] | 文本类型的NDEF Record。 |
| RTD_URI 9+ | number[] | [0x55] | URI类型的NDEF Record。 |
...
| 类型 | 说明 |
| --- | --- |
| _TagSession | TagSession是所有 NFC Tag技术类型 的基类， 提供建立连接和发送数据等共同接口。 |