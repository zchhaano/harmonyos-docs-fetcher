...
## wifiManager.getScanInfoList 10+
...
| bssidType 10+ | DeviceAddressType | 否 | 否 | 热点的BSSID类型。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| channelWidth | number | 否 | 否 | WLAN接入点的带宽，具体定义参见 WifiChannelWidth 。 |
...
| supportedWifiCategory 12+ | WifiCategory | 否 | 否 | 热点支持的最高Wi-Fi级别。 |
| isHiLinkNetwork 12+ | boolean | 否 | 否 | 热点是否支持hiLink，true表示支持， false表示不支持。 |
...
| bssidType 10+ | DeviceAddressType | 否 | 是 | 热点的BSSID类型。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| netId 22+ | number | 否 | 是 | 分配的网络ID。 |
| securityType | WifiSecurityType | 否 | 否 | 加密类型。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| eapConfig 10+ | WifiEapConfig | 否 | 是 | 可扩展身份验证协议配置。只有securityType为WIFI_SEC_TYPE_EAP时需要填写。 |
| wapiConfig 12+ | WifiWapiConfig | 否 | 是 | WAPI身份验证协议配置。只有securityType为WIFI_SEC_TYPE_WAPI_CERT或WIFI_SEC_TYPE_WAPI_PSK时需要填写。 |
...
## wifiManager.connectToCandidateConfigWithUserAction 20+
...
## wifiManager.addDeviceConfig 15+
...
## wifiManager.getDeviceConfigs 15+
...
## wifiManager.connectToNetwork 15+
...
## wifiManager.getLinkedInfoSync 18+
...
| rxLinkSpeed 10+ | number | 否 | 否 | WLAN接入点的下行速度，单位Mbps。 |
| maxSupportedTxLinkSpeed 10+ | number | 否 | 否 | 当前支持的最大上行速率，单位Mbps。 |
| maxSupportedRxLinkSpeed 10+ | number | 否 | 否 | 当前支持的最大下行速率，单位Mbps。 |
| frequency | number | 否 | 否 | WLAN接入点的频率。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| ipAddress | number | 否 | 否 | WLAN连接的IP地址。 1. IP地址在WiFi连接信息和"设置 > 关于本机 > 状态信息"中可以查看。 2. ipAddress值为number类型，需要转换为IP常用格式，具体请参考 IP格式转换 。 |
| connState | ConnState | 否 | 否 | WLAN连接状态。 |
| channelWidth 10+ | WifiChannelWidth | 否 | 否 | 当前连接热点的信道带宽。 |
| wifiStandard 10+ | WifiStandard | 否 | 否 | 当前连接热点的Wi-Fi标准。 |
| supportedWifiCategory 12+ | WifiCategory | 否 | 否 | 热点支持的最高Wi-Fi级别。 |
| isHiLinkNetwork 12+ | boolean | 否 | 否 | 热点是否支持hilink，true表示支持， false表示不支持。 |
| wifiLinkType 18+ | WifiLinkType | 否 | 是 | Wi-Fi7连接类型。 |
...
## wifiManager.getDeviceMacAddress 15+
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| ipAddress | number | 否 | 否 | IP地址。（ipAddress值为number类型，需要转换为IP常用格式，具体请参考 IP格式转换 ）。 |
...
| uniqueIpv6Address 12+ | string | 否 | 是 | 唯一本地Ipv6地址。 |
| randomUniqueIpv6Address 12+ | string | 否 | 是 | 随机唯一本地Ipv6地址。 |
...
## wifiManager.isBandTypeSupported 10+
...
## wifiManager.isMeteredHotspot 11+
...
## wifiManager.isHotspotActive 15+
...
| deviceAddressType 10+ | DeviceAddressType | 否 | 是 | 设备MAC地址类型。 |
...
| deviceAddressType 10+ | DeviceAddressType | 否 | 是 | 设备地址类型。 |
...
## wifiManager.getMultiLinkedInfo 18+
