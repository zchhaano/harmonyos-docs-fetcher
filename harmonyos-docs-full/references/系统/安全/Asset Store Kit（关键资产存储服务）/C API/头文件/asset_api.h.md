...
| 名称 | 描述 |
| --- | --- |
| int32_t OH_Asset_Add(const Asset_Attr *attributes, uint32_t attrCnt) | 新增一条关键资产。 如果要设置 Asset_Tag .ASSET_TAG_IS_PERSISTENT属性，需要申请ohos.permission.STORE_PERSISTENT_DATA权限。 |
...
如果要设置[Asset_Tag](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-asset-type-h#asset_tag).ASSET_TAG_IS_PERSISTENT属性，需要申请ohos.permission.STORE_PERSISTENT_DATA权限。
...
| Asset_Blob *challenge | 挑战值，在后续调用 OH_Asset_Query 时使用。 |
...
| 参数项 | 描述 |
| --- | --- |
| const Asset_Attr *handle | 待处理的查询句柄，当前包含 OH_Asset_PreQuery 执行成功返回的挑战值。 |
...
| 参数项 | 描述 |
| --- | --- |
| const Asset_Result *result | 从 OH_Asset_Query 中获取的查询结果。 |
...
| 参数项 | 描述 |
| --- | --- |
| Asset_Blob *blob | 从 OH_Asset_PreQuery 获取的挑战值。 |
...
| 参数项 | 描述 |
| --- | --- |
| Asset_ResultSet *resultSet | 从 OH_Asset_Query 得到的查询结果列表。 |