...
| ParamList OH_HiAppEvent_CreateParamList(void) | - | 创建一个指向参数列表对象的指针。 注意 ：创建的指向参数列表对象的指针不再使用后，必须通过 OH_HiAppEvent_DestroyParamList 接口进行销毁。 |
...
| HiAppEvent_Watcher* OH_HiAppEvent_CreateWatcher(const char* name) | - | 创建一个用于监听app事件的监听器。 注意 ：创建的监听器不再使用后，必须通过 OH_HiAppEvent_DestroyWatcher 接口进行销毁。 |
...
| HiAppEvent_Processor* OH_HiAppEvent_CreateProcessor(const char* name) | - | 创建一个用于处理app事件上报的处理者。 注意 ：创建的处理者不再使用后，必须通过 OH_HiAppEvent_DestroyProcessor 接口进行销毁。 |
...
| HiAppEvent_Config* OH_HiAppEvent_CreateConfig(void) | - | 创建一个指向设置系统事件触发条件的配置对象的指针。 注意 ：创建的指向设置系统事件触发条件的配置对象的指针不再使用后，必须通过 OH_HiAppEvent_DestroyConfig 接口进行销毁。 |
...
| int OH_HiAppEvent_SetEventConfig(const char* name, HiAppEvent_Config* config) | - | 事件相关的配置参数设置方法。 不同的事件有不同的配置项，目前仅支持以下事件： MAIN_THREAD_JANK（参数配置详见 主线程超时事件检测 ） MAIN_THREAD_JANK_V2（参数配置详见 主线程超时事件检测 ） |
...
创建的指向参数列表对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyParamList](/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_destroyparamlist)接口进行销毁。
...
| enum EventType type | 事件类型，在 EventType 中定义。 |
...
| 参数项 | 描述 |
| --- | --- |
| const char* name | 配置项名称。名称可填 DISABLE 和 MAX_STORAGE 。 |
| const char* value | 配置项值。如果配置项名称是 DISABLE ，值可以填“true”或者“false”； 如果配置项名称是 MAX_STORAGE ，配额值字符串只由数字字符和大小单位字符（单位字符支持[b\|k\|kb\|m\|mb\|g\|gb\|t\|tb]，不区分大小写）构成。 配额值字符串必须以数字开头，后面可以选择不传单位字符（默认使用byte作为单位），或者以单位字符结尾。 |
...
创建的监听器不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_destroywatcher)接口进行销毁。
...
创建的处理者不再使用后，必须通过[OH_HiAppEvent_DestroyProcessor](/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_destroyprocessor)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS ：接口调用成功； HIAPPEVENT_PROCESSOR_IS_NULL ：processor入参为空； HIAPPEVENT_INVALID_PARAM_VALUE ：参数值无效； HIAPPEVENT_INVALID_UID ：用户标识无效； HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH ：参数值长度无效。 具体可参考 HiAppEvent_ErrorCode 。 |
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS ：接口调用成功； HIAPPEVENT_PROCESSOR_IS_NULL ：processor入参为空； HIAPPEVENT_INVALID_PARAM_VALUE ：参数值无效； HIAPPEVENT_INVALID_UID ：用户标识无效。 具体可参考 HiAppEvent_ErrorCode 。 |
...
| 类型 | 说明 |
| --- | --- |
| int64_t | 调用成功时返回处理者唯一ID，大于0； HIAPPEVENT_PROCESSOR_IS_NULL ：processor入参为空； HIAPPEVENT_INVALID_PARAM_VALUE ：参数值无效； HIAPPEVENT_OPERATE_FAILED ：数据处理者名称未找到或注册失败； HIAPPEVENT_INVALID_UID ：用户标识无效。 具体可参考 HiAppEvent_ErrorCode 。 |
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS ：接口调用成功； HIAPPEVENT_PROCESSOR_NOT_FOUND ：事件处理者不存在； HIAPPEVENT_OPERATE_FAILED ：操作失败； HIAPPEVENT_INVALID_UID ：用户标识无效。 具体可参考 HiAppEvent_ErrorCode 。 |
...
创建的指向设置系统事件触发条件的配置对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyConfig](/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_destroyconfig)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS ：接口调用成功； HIAPPEVENT_EVENT_CONFIG_IS_NULL ：传入的指向配置对象的指针为空； HIAPPEVENT_INVALID_PARAM_VALUE ：设定的配置项无效。 具体可参考 HiAppEvent_ErrorCode 。 |
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS ：接口调用成功； HIAPPEVENT_INVALID_PARAM_VALUE ：设置的参数无效。 具体可参考 HiAppEvent_ErrorCode 。 |