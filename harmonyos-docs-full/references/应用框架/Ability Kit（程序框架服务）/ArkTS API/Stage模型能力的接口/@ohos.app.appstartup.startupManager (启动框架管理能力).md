...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTasks | Array<string> | 是 | 表示准备执行的启动任务 StartupTask 的名称或预加载so名称的数组。 |
...
| --- | --- | --- | --- |
| startupTasks | Array<string> | 是 | 表示准备执行的启动任务 StartupTask 的名称或预加载so名称的数组。 |
| context | common.AbilityStageContext | 是 | 表示执行启动任务 StartupTask 的AbilityStage上下文，作为入参传给启动任务的 init 。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTask | string | 是 | 启动任务 StartupTask 的名称或预加载so名称。 |
...
| 类型 | 说明 |
| --- | --- |
| Object | 输入为启动任务名时，返回指定的启动任务 init 返回的执行结果。 输入为so文件名时，返回undefined。 |
