...
| globalDisplayX 20+ | number | 否 | 是 | 手指位置相对于全局屏幕的左上角的X坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
| globalDisplayY 20+ | number | 否 | 是 | 手指位置相对于全局屏幕的左上角的Y坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |

## onAccessibilityHoverTransparent 20+
...
## AccessibilityTransparentCallback 20+
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | TouchEvent | 是 | 原始touch事件。 说明： TouchEvent对象的触摸事件的类型 TouchType 为四种无障碍悬浮事件类型中的一种，四种无障碍悬浮事件类型为HOVER_ENTER、HOVER_MOVE、HOVER_EXIT和HOVER_CANCEL。 |
