...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| totalCount | number | 否 | 是 | 加载的数据项总数，可以不等于数据源长度。 取值范围：[0, +∞) 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| reusable 18+ | boolean | 否 | 是 | 是否开启复用功能。true代表开启复用，false代表关闭复用。 默认值：true 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| onLazyLoading 19+ | (index: number) => void | 否 | 是 | 数据懒加载函数，向指定的数据源index中写入数据。 元服务API： 从API version 19开始，该接口支持在元服务中使用。 |
| onTotalCount 19+ | () => number | 否 | 是 | 数据项总数计算函数，返回值可以不等于数据源长度。推荐使用onTotalCount代替totalCount。同时设置totalCount与onTotalCount时，忽略totalCount。 取值范围：[0, +∞) 元服务API： 从API version 19开始，该接口支持在元服务中使用。 |
