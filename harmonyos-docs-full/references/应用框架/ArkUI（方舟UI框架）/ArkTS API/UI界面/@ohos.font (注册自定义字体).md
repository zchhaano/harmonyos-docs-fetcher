...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| familyName | string \| Resource 10+ | 否 | 否 | 设置注册的字体名称。 |
| familySrc | string \| Resource 10+ | 否 | 否 | 设置注册字体文件的路径。 说明： 读取系统沙箱路径内的资源时，建议使用file://路径前缀的字符串，需要确保沙箱目录路径下的文件存在并且有可读权限。 |
...
| weight | number | 否 | 否 | 系统字体的字重。 取值范围：[100,900]，取值间隔为100，分别对应 FontWeight 枚举中的值。 默认值：100 |
| width | number | 否 | 否 | 系统字体的宽度。 取值范围：[1,9]，取值间隔为1，分别对应 FontWidth 枚举中的值。 |
