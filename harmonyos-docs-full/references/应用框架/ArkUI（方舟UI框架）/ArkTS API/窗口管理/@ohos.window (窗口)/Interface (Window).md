...
- 若需在非自由窗口状态下实现相对于屏幕的移动，请使用[moveWindowToGlobal()](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#movewindowtoglobal15)。
- 该方法对非自由窗口状态下的主窗口无效。
...
窗口容器背景色覆盖整个窗口区域，包括标题栏和内容区域。当同时使用该接口和[setWindowBackgroundColor()](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowbackgroundcolor9)设置背景色时，内容区域显示窗口背景色，标题栏显示窗口容器背景色。
...
## getWindowAvoidAreaIgnoringVisibility 22+
...
- 主窗口在非[自由窗口](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#自由窗口)状态的自由悬浮窗口模式（即窗口模式为window.WindowStatusType.FLOATING）下，仅存在系统栏（[AvoidAreaType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-e#avoidareatype7)为TYPE_SYSTEM）类型的避让区域。
- 主窗口在其余场景下，仅当在非自由悬浮窗口模式下或设备类型为Phone和Tablet，才能通过此接口获取计算后的避让区域，否则获取的避让区域为空。
- 子窗口在非自由窗口状态或非自由悬浮窗口模式下，仅当窗口的位置和大小与主窗口一致时，才能通过此接口获取计算后的避让区域，否则获取的避让区域为空。
...
## getImmersiveModeEnabledState 12+
...
## setSpecificSystemBarEnabled 11+
...
| enableAnimation 12+ | boolean | 否 | 设置状态栏或底部导航区域显示状态变化时是否使用动画，true表示使用， false表示不使用，默认值为false。 |
...
## setWindowSystemBarProperties 9+
...
## getWindowSystemBarProperties 12+
...
- [自由窗口](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#自由窗口)状态的自由悬浮窗口模式（即窗口模式为window.WindowStatusType.FLOATING）下触发回调时，仅存在固定态软键盘（[AvoidAreaType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-e#avoidareatype7)为TYPE_KEYBOARD）类型的避让区域。
...
| callback | Callback< OcclusionState > | 是 | 窗口可见性变化时的回调函数。详情见 可见性状态 。 |
...
| callback | Callback< FrameMetrics > | 是 | 窗口帧率指标变化时的回调函数。详情见 帧率指标 。 |
...
| callback | RotationChangeCallback< RotationChangeInfo , RotationChangeResult \| void> | 是 | 回调函数。返回窗口旋转信息 RotationChangeInfo ，应用返回当前窗口变化结果 RotationChangeResult 。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | string \| ColorMetrics 18+ | 是 | 需要设置的背景色，为十六进制RGB或ARGB颜色，不区分大小写，例如'#00FF00'或'#FF00FF00'。 从API version 18开始，此参数支持ColorMetrics类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isFocusable | boolean | 是 | 窗口是否可获焦。true表示支持；false表示不支持。设置为false时，该窗口不支持绑定输入法和接收键盘事件，如需处理输入逻辑，建议参考 不可获焦窗口中输入框与输入法交互指南 。 |
| callback | AsyncCallback<void> | 是 | 回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ratio | number | 是 | 窗口内容布局（不含边框和标题栏等装饰）的宽高比。该参数为浮点数，受窗口最大最小尺寸限制，比例值下限为最小宽度/最大高度，上限为最大宽度/最小高度。窗口最大最小尺寸由 WindowLimits 和系统限制的交集决定，系统限制优先级高于 WindowLimits 。ratio的有效范围会随 WindowLimits 变化而变化。如果先设置了 WindowLimits ，后设置的ratio与其冲突，会返回错误码；如果先设置了ratio，后设置的 WindowLimits 与其冲突，窗口的宽高比可能会不跟随设置的宽高比（ratio）。 |
...
| --- | --- | --- | --- |
| ratio | number | 是 | 窗口内容布局（不含边框和标题栏等装饰）的宽高比。该参数为浮点数，受窗口最大最小尺寸限制，比例值下限为最小宽度/最大高度，上限为最大宽度/最小高度。窗口最大最小尺寸由 WindowLimits 和系统限制的交集决定，系统限制优先级高于 WindowLimits 。ratio的有效范围会随 WindowLimits 变化而变化。如果先设置了 WindowLimits ，后设置的ratio与其冲突，会返回错误码；如果先设置了ratio，后设置的 WindowLimits 与其冲突，窗口的宽高比可能会不跟随设置的宽高比（ratio）。 |
| isPersistent | boolean | 否 | 是否持久化保存该比例参数。 如为true，比例参数会持久化保存，销毁窗口、关闭应用或重启设备后，当再次切换到自由悬浮窗口模式时仍然生效。可通过 resetAspectRatio 清除持久化保存的比例参数。 如为false，比例参数仅对当前窗口生效，窗口销毁后清除该数据。 默认值为true。 |
| needUpdateRect | boolean | 否 | 是否立即根据当前比例更新窗口大小。 如为true，立即根据当前比例更新窗口大小。 如为false，窗口将在拖拽缩放时根据当前比例更新，也可以使用 resize 或 resizeAsync 进行主动更新。 默认值为true。 |
...
| --- | --- | --- | --- |
| windowLimits | WindowLimits | 是 | 目标窗口的尺寸限制，单位为px或vp。 |
| isForcible | boolean | 是 | 是否强制设置窗口的尺寸限制。 入参 windowLimits 的单位为vp时：无论设置true还是false，都按照false处理，窗口宽高的最小值和最大值都取决于系统限制。 入参 windowLimits 的单位为px时：设置为true，表示窗口宽高最小值以系统限制值和40vp两者中的低数值为准，窗口宽高的最大值仍取决于系统限制；设置为false，表示窗口宽高的最小值和最大值都取决于系统限制。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise< WindowLimits > | Promise对象。返回设置后的窗口尺寸限制。 入参 windowLimits 的单位为vp时，返回入参与系统默认窗口尺寸限制的交集。 入参 windowLimits 的单位为px时，isForcible为false则返回入参与系统默认窗口尺寸限制的交集；isForcible为true则返回入参与[系统限制的最小值与40vp两者中的低数值，系统限制的最大值]的交集。 |
...
设置窗口标题，存在标题栏的窗口形态生效，若不存在标题栏则返回1300002错误码，使用Promise异步回调。如果使用Stage模型，该接口需要在[loadContent()](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#loadcontent9)或[setUIContent()](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setuicontent9)调用生效后使用。
...
此接口仅支持设置子窗口模态类型，当需要禁用子窗口模态属性时，建议使用[setSubWindowModal 12+](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setsubwindowmodal12)。
...
| options | SubWindowOptions | 是 | 子窗口参数。decorEnabled为true时，子窗口为非 沉浸式布局 ；decorEnabled为false时，子窗口为沉浸式布局。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| windowId | number | 是 | 父窗口id，该参数应为整数。推荐使用 getWindowProperties() 方法获取父窗口id属性。 |
...
## setWindowTitleButtonVisible 14+
...
## disableLandscapeMultiWindow 12+
...
## setDialogBackGestureEnabled 12+
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| offsetX | number | 是 | 窗口移动时预期鼠标位置相对窗口左上角的x轴偏移量，单位为px，该参数仅支持整数输入，浮点数向下取整。负值为非法参数，大于窗口宽度为非法参数，窗口宽度可以在窗口属性 WindowProperties 中获取。 |
| offsetY | number | 是 | 窗口移动时预期鼠标位置相对窗口左上角的y轴偏移量，单位为px，该参数仅支持整数输入，浮点数向下取整。负值为非法参数，大于窗口高度为非法参数，窗口高度可以在窗口属性 WindowProperties 中获取。 |
...
## getWindowCornerRadius 17+

 支持设备PhonePC/2in1TabletTVWearable

getWindowCornerRadius(): number

该接口用于获取子窗或悬浮窗的圆角半径值，在未调用[setWindowCornerRadius()](/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowcornerradius17)接口设置窗口圆角半径值时，调用此接口可获取窗口默认圆角半径值。
...
## setFollowParentMultiScreenPolicy 17+
...
## setFollowParentWindowLayoutEnabled 17+
...
## setRelativePositionToParentWindowEnabled 20+
...
## setWindowTransitionAnimation 20+
...
## getWindowTransitionAnimation 20+
...
| 类型 | 说明 |
| --- | --- |
| TransitionAnimation \| undefined | 对应场景下的转场动画配置。当未使用过 setWindowTransitionAnimation 接口时，返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| width | number | 是 | 当前窗口的目标宽度，单位为px，该参数仅支持整数输入，浮点数输入将向下取整，负值为非法参数（抛出错误码 401 ）。 |
| height | number | 是 | 当前窗口的目标高度，单位为px，该参数仅支持整数输入，浮点数输入将向下取整，负值为非法参数（抛出错误码 401 ）。 |
| callback | AsyncCallback<void> | 是 | 回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isFocusable | boolean | 是 | 点击时是否支持切换焦点窗口。true表示支持；false表示不支持。设置为false时，该窗口不支持绑定输入法和接收键盘事件，如需处理输入逻辑，建议参考 不可获焦窗口中输入框与输入法交互指南 。 |
| callback | AsyncCallback<void> | 是 | 回调函数。 |
