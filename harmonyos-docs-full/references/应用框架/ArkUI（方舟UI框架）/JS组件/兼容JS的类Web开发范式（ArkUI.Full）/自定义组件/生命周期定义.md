...
```
<!-- comp.hml --> < div class = "item" > < text class = "text-style" > {{ value }} </ text > </ div >
```

 收起自动换行深色代码主题复制

```
//comp.js export default { data : { value : "组件创建" }, onInit ( ) { console . info ( "组件创建" ) }, onAttached ( ) { this . value = "组件挂载" , console . info ( "组件挂载" ) }, onShow ( ) { console . info ( "Page显示" ) }, onHide ( ) { console . info ( "Page隐藏" ) } }
```