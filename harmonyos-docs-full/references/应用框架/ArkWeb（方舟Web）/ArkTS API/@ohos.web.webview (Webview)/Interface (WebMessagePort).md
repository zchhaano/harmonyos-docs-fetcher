...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| isExtentionType 10+ | boolean | 否 | 是 | 创建WebMessagePort时是否指定使用扩展增强接口， postMessageEventExt 、 onMessageEventExt 。 true表示使用扩展增强接口，false表示不使用扩展增强接口。 默认值：false。 |
