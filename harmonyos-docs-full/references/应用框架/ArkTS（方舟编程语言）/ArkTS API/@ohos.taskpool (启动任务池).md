...
- 周期任务：由[executePeriodically](#taskpoolexecuteperiodically12)执行的任务。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ac/v3/1lFqwRfzRB-5cI0XB-QBRg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=CE36A899AC631E5A23D9015351C75E75FF9C05303CCD323E86B06EBC7C09D6C0)   
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 执行的逻辑需要传入一个函数，该函数必须使用 @Concurrent装饰器 装饰。支持的函数返回值类型请参考 序列化支持类型 。 |
| args | Object[] | 否 | 执行逻辑的函数所需要的入参，支持的参数类型请参考 序列化支持类型 。默认值为undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | (...args: A) => R \| Promise<R> | 是 | 执行的逻辑需要传入函数，必须使用 @Concurrent装饰器 装饰，支持的函数返回值类型请参考 序列化支持类型 。 |
| args | A | 否 | 执行逻辑的函数所需要的入参，支持的参数类型请参考 序列化支持类型 。默认值为undefined。 |
...
#### taskpool.executePeriodically 12+
...
#### taskpool.executePeriodically 13+
...
通过taskId或taskId与taskName获取对应的Task实例。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/53/v3/fKnjUHpYS2yDXEBZAZCAWg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=B48ACDDF3BDAA14BD71630707024723841CAE92A0AF78DC3EA14345F781E95B1)   
...
| IDLE 12+ | 3 | 任务为后台任务。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| function | Function | 否 | 否 | 创建任务时需要传入的函数，支持的函数返回值类型请参考 序列化支持类型 。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| arguments | Object[] | 否 | 是 | 创建任务传入函数所需的参数，支持的参数类型请参考 序列化支持类型 。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| name 11+ | string | 否 | 否 | 创建任务时指定的任务名称。不建议修改此值。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| taskId 18+ | number | 否 | 否 | 任务的ID。任务的标识符，系统默认提供全局唯一值，不建议修改此值。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| totalDuration 11+ | number | 否 | 否 | 执行任务总耗时。单位为ms。不建议修改此值。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| ioDuration 11+ | number | 否 | 否 | 执行任务异步IO耗时。单位为ms。不建议修改此值。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| cpuDuration 11+ | number | 否 | 否 | 执行任务CPU耗时。单位为ms。不建议修改此值。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 执行的逻辑需要传入函数，必须使用 @Concurrent装饰器 装饰，支持的函数返回值类型请参考 序列化支持类型 。 |
| args | Object[] | 否 | 任务执行传入函数的入参，支持的参数类型请参考 序列化支持类型 。默认值为undefined。 |
...
| name | string | 是 | 任务名称。 |
| func | Function | 是 | 执行的逻辑需要传入函数，必须使用 @Concurrent装饰器 装饰，支持的函数返回值类型请参考 序列化支持类型 。 |
| args | Object[] | 否 | 任务执行时传入函数的参数。支持的类型请参考 序列化支持类型 。默认值为undefined。 |
...
```
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b4/v3/7ctAogSnTxCvX-vouex-Ig/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=05A94548A2BD4B2DEB429A251D40904DFDA3BF31E904585065F4DF23FFCC349F)   
...
设置任务的传输列表。使用该方法前需要先构造Task。不调用该接口，则传给任务的数据中的ArrayBuffer默认transfer转移。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ac/v3/hNxdSzGxRz2ROMsqy4Q5rQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=DD0543D784C29AD385E6931D3ED95B076F14CE58C1A0A82508FB424E213CA685)   
...
设置任务的拷贝列表。在使用该方法前，需先构造Task对象。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/18/v3/LNQDTnRzS4u159zPVxgUwQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=645BD5FB8F2CE24E5B6E396DECC98763BF8A0963A5FE7B9F51E9C25942CCBC9A)   
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| cloneList | Object[] \| ArrayBuffer[] | 是 | - 传入数组的类型必须为 Sendable支持的数据类型 或ArrayBuffer。 - 所有传入cloneList的对象持有的 Sendable class 实例或ArrayBuffer类型对象，在线程间传输的行为都会变成拷贝传递，即修改传输后的对象不会对原有对象产生任何影响。 |
...
任务执行过程中向宿主线程发送消息并触发回调。使用此方法前需构造Task。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a/v3/fMYTvyaqQfOmczIqg3e38A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=84F3EC0AB72F0C1B47684DEE87CACD774065AB5692270487749BD2F7DDE9D850)   
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| args | Object[] | 否 | 可传输对象默认转移，作为回调函数的参数。支持的参数类型请参见 序列化支持类型 ，默认值为undefined。 |
...
为任务注册回调函数，接收并处理任务池工作线程的数据。使用此方法前，需构造Task。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/WoSB__UGSxKPrIDsAaNLdw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=135FC36FE6BACEE2C2A936B2FBA210602672CB2460F916D811F612FB4BF3F402)   
...
| --- | --- | --- | --- |
| func | (...args: A) => R \| Promise<R> | 是 | 执行的逻辑需要传入函数，必须使用 @Concurrent装饰器 装饰，支持的函数返回值类型请参考 序列化支持类型 。 |
| args | A | 否 | 任务执行传入函数的入参，支持的参数类型请参考 序列化支持类型 。默认值为undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 需要传入使用 @Concurrent装饰器 装饰的函数。支持的返回值类型请参考 序列化支持类型 。 |
| args | Object[] | 否 | 任务执行函数的入参，支持的类型请参考 序列化支持类型 ，默认值为undefined。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| name 11+ | string | 否 | 否 | 创建任务组时指定的任务组名称。 |
...
SequenceRunner的构造函数。构造一个全局串行队列，如果名字相同，将返回同一个串行队列。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b9/v3/qxGZ1AvxTieC-YWxGhfzyQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=4BF8162BD4240B541CCA565059CD1E2DD435D334B8B819A394574C176F061D5E)   
...
执行串行任务。使用该方法前需先构造SequenceRunner。串行队列不能执行任务组任务、其他串行队列任务、异步队列任务、有依赖关系的任务和已执行的任务。使用Promise异步回调。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/qNRz6TQ2R9Cj3nRLBQVq-A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=8EBF4136617DA22DFB88F799623267C339273A88802485DE3D361E0D53A38373)   
...
AsyncRunner的构造函数用于构造一个全局异步队列。如果队列名称相同，将返回同一个异步队列实例。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/58/v3/35s6ikDiQG2dATHD-VnGNA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=752E0CAC94159CF9C5B413443CEBA702C179D8C9F48634A74B8168CC0217CBA2)   
...
执行异步任务。使用该方法前需要先构造AsyncRunner。使用Promise异步回调。
  ...
  ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bb/v3/GJE7dfCWTUSUGMjjlcZ9PA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=F82A95105A88B1C18C5D0A26D1FD3209BFB466446287E03F157957676CFA4A9B)   
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| name 12+ | string | 否 | 否 | 任务的名字，不建议修改此值。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| error | Error \| Object | 否 | 是 | 错误信息。默认和BusinessError的message字段一致。不建议修改此值。 |
   ...
   ![image](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/30/v3/V57N2L8kTVy_6KuCHqGmdw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260420T194257Z&HW-CC-Expire=86400&HW-CC-Sign=C865610C17B648FF4BCBD26CCF3F9811ECFCEE64650D182F5C5F34D22A654295)   
