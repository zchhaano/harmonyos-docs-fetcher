...
```
POST / order / harmony / v1 / application / order / status / query Content - Type : application / json ; charset = UTF - 8 Authorization : Bearer ***.***.*** Accept : application / json { "purchaseToken" : "***.*.***" , "purchaseOrderId" : "***.***" }
```
...
```
HTTP / 1.1 200 OK Content - Type : application / json ; charset = UTF - 8 { "responseCode" : "0" , "responseMessage" : "success" , "jwsPurchaseOrder" : "***" }
```