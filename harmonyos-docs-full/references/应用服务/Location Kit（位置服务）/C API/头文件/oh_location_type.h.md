...
| 参数项 | 描述 |
| --- | --- |
| Location_Info * location | 指向位置信息结构体的指针。 需要传入非空指针，该指针可以在 Location_InfoCallback 中获取。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_BasicInfo | 返回位置基本信息结构体。详细定义请参考 Location_BasicInfo 。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_ResultCode | 返回操作结果，详细定义参见 Location_ResultCode 。 LOCATION_SUCCESS 获取附加信息成功。 LOCATION_INVALID_PARAM 1. 入参location或additionalInfo是空指针。 2. 入参length太小，也就是additionalInfo指向的内存太小导致无法保存完整的附加信息字符串。 |
...
| 参数项 | 描述 |
| --- | --- |
| Location_Info * location | 指向 Location_Info 实例的指针，携带最新的位置信息。 location实例的内存会在 Location_InfoCallback 结束时回收，请在此之前调用 OH_LocationInfo_GetBasicInfo 等接口获取位置信息。 |
| void* userData | 指向调用者数据结构或对象的指针，该参数是通过 OH_LocationRequestConfig_SetCallback 传入的。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_RequestConfig* | 返回指向 Location_RequestConfig 实例的指针。 如果返回NULL表示创建失败，可能的原因是应用地址空间满，导致空间分配不出来。 |
...
| 参数项 | 描述 |
| --- | --- |
| Location_RequestConfig * requestConfig | 指向 Location_RequestConfig 实例的指针。 该实例是由 OH_Location_CreateRequestConfig 创建的。 |
...
| --- | --- |
| Location_RequestConfig * requestConfig | 指向 Location_RequestConfig 实例的指针。 该实例是由 OH_Location_CreateRequestConfig 创建的。 |
| Location_UseScene useScene | 表示位置请求时的用户活动场景。 默认值是 LOCATION_USE_SCENE_DAILY_LIFE_SERVICE 。 详细定义见 Location_UseScene 。 |
...
| --- | --- |
| Location_RequestConfig * requestConfig | 指向 Location_RequestConfig 实例的指针。 该实例是由 OH_Location_CreateRequestConfig 创建的。 |
| Location_PowerConsumptionScene powerConsumptionScene | 表示位置请求的功耗场景。 默认值是 LOCATION_LOW_POWER_CONSUMPTION 。 详细定义见 Location_PowerConsumptionScene 。 |
...
| --- | --- |
| Location_RequestConfig * requestConfig | 指向 Location_RequestConfig 实例的指针。 该实例是由 OH_Location_CreateRequestConfig 创建的。 |
| Location_InfoCallback callback | 指向回调函数的指针，该回调函数用于接收位置信息变化。 详细定义请参考 Location_InfoCallback 。 |
